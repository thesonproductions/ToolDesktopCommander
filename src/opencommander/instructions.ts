/** Workflow guidance delivered to the model (MCP `instructions`, session_start, skill file). */

export const WORKFLOW_GUIDE = `OpenCommander — coding-agent workflow (follow strictly)
...
export const SHORT_INSTRUCTIONS = `OpenCommander gives you a real development machine. Call session_start first. Use job_start/job_wait for anything slow (never block), request_key for idempotency, git_checkpoint before edits, repo_verify after edits, and task_state_save to survive disconnects.`;
